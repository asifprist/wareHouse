...
@Component
public class UserFCDFilter extends OncePerRequestFilter {
	
	   @Autowired
	    private JwtToken jwtToken;

	    @Autowired
	    private UserInfoUserDetailsService userDetailsServices;
...
//	           jwtToken.extractUsername(token);
//	        }
	        if (authHeader != null && authHeader.startsWith("Bearer ")) {
	            token = authHeader.substring(7);
	            username = jwtToken.extractUsername(token);  // Set the username
	        }
...
//	@Autowired
//	private UserFCDservice userfcdservice;
//	@Autowired
//	private JwtToken jwtToken;
//
//	@Override
...
//			if (userName!=null && SecurityContextHolder.getContext().getAuthentication()==null) {
//				UserDetails userDetails = userfcdservice.loadUserByUsername(userName);
